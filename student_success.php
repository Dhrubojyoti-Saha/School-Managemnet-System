...
if(isset($_POST['submit']) && $_POST['submit'] == 'Add New'){
	...
	if($Name !='' && $MobileNo !='' && $EmailId !='' && $Address !='' && $SubjectName !='' && $Password !='' ){
		   ...
		   $FlatConn -> query("INSERT INTO `student_tbl` SET `name` = '".$Name."', `mobile_no` = '".$MobileNo."', `email_id` = '".$EmailId."', `address` = '".$Address."', `subject` = '".$SubjectName."', `password` = '".$Password."', `created_at` = '".date('Y-m-d', time())."'");
